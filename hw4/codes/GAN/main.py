...
if __name__ == "__main__":
    ...
    config = "z-{}_hidden-dim-{}_batch-{}_num-train-steps-{}".format(
        args.latent_dim,
    ...
    if args.do_train:
        ...
        trainer = Trainer(
            device, netG, netD, optimG, optimD, dataset, args.ckpt_dir, tb_writer
        )
        trainer.train(args.num_training_steps, args.logging_steps, args.saving_steps)

    restore_ckpt_path = os.path.join(
        args.ckpt_dir, str(max(int(step) for step in os.listdir(args.ckpt_dir)))
    )
