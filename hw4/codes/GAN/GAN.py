...
class Generator(nn.Module):
    def __init__(self, num_channels, latent_dim, hidden_dim):
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(
                latent_dim, 4 * hidden_dim, kernel_size=4, stride=1, padding=0
            ),
            nn.BatchNorm2d(4 * hidden_dim),
            nn.ReLU(),
            nn.ConvTranspose2d(
                4 * hidden_dim, 2 * hidden_dim, kernel_size=4, stride=2, padding=1
            ),
            nn.BatchNorm2d(2 * hidden_dim),
            nn.ReLU(),
            nn.ConvTranspose2d(
                2 * hidden_dim, hidden_dim, kernel_size=4, stride=2, padding=1
            ),
            nn.BatchNorm2d(hidden_dim),
            nn.ReLU(),
            nn.ConvTranspose2d(
                hidden_dim, num_channels, kernel_size=4, stride=2, padding=1
            ),
            nn.Tanh(),
        )
