...
class Trainer(object):
    def __init__(
        self, device, netG, netD, optimG, optimD, dataset, ckpt_dir, tb_writer
    ):
        self._device = device
        self._netG = netG
        self._netD = netD
    ...
    def train_step(self, real_imgs, fake_imgs, BCE_criterion):
        ...
        *   Returns:
        ...
        loss_D_real = BCE_criterion(D_real, torch.ones_like(D_real))
        D_x = D_real.mean().item()
        loss_D_real.backward()

        # ** accumulate ** the gradients of binary_cross_entropy(netD(fake_imgs), 0) w.r.t. netD
        # record average D(fake_imgs)
        D_fake_1 = self._netD(fake_imgs)
        loss_D_fake = BCE_criterion(D_fake_1, torch.zeros_like(D_fake_1))
        D_G_z1 = D_fake_1.mean().item()
        ...
        # record average D(fake_imgs)
        D_fake_2 = self._netD(fake_imgs)
        loss_G = BCE_criterion(D_fake_2, torch.ones_like(D_fake_2))
        D_G_z2 = D_fake_2.mean().item()
        loss_G.backward()
