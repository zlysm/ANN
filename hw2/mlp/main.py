...
def train_epoch(model, X, y, optimizer):  # Training Process
    ...
    while st < len(X) and ed <= len(X):
        optimizer.zero_grad()
        X_batch, y_batch = torch.from_numpy(X[st:ed]).to(device), torch.from_numpy(
            y[st:ed]
        ).to(device)
...
if __name__ == "__main__":
    ...
    if args.is_train:
        ...
        ax[0].plot(loss[0], label="train")
        ax[0].plot(loss[1], label="valid")
        ax[0].set_ylabel("loss")
        ax[0].legend()
        ax[1].plot(acc[0], label="train")
        ax[1].plot(acc[1], label="valid")
        ax[1].set_ylabel("accuracy")
        ax[1].legend()
        fig.suptitle("MLP on CIFAR-10")
        plt.show()
